...
function PerfumeDetailPage() {
    ...
    const handleAddToCart = () => {
        addCart(perfume);
        alert(`${perfume.name} fue añadido al carrito.`)
    }
                    ...
                    <div className="w-full text-start flex flex-col gap-5">
                        ...
                        <div className='text-center flex flex-col gap-5 justify-between items-center'>
                            ...
                            <div className='self-end flex flex-col gap-5'>
                                ...
                                <a href={'https://api.whatsapp.com/send?phone=51960153257&text=Hola, estoy interesado en ' + encodeURIComponent(perfume.name) + ' ' + encodeURIComponent(perfume.version) + ` ${encodeURIComponent(window.location.href)}`} target='_blank' rel='noreferrer' className="bg-[#25D366] rounded-lg w-fit py-2 px-5 self-center font-bold hover:scale-95 uppercase transition-transform text-white shadow-sm hover:bg-white border hover:border-[#25D366] hover:text-[#25D366] text-xl flex gap-3 items-center">
